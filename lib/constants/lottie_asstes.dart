
List<String> lottie =[
  "assets/lottie/lottie_heart.json",
  "assets/lottie/lottie_kidney.json",
  "assets/lottie/lottie_liver.json",
  "assets/lottie/lottie_lungs.json",
  "assets/lottie/lottie_nose.json",
  "assets/lottie/lottie_eye.json",
];
List<String> lottieName =[
  "Heart",
  "Kidney",
  "Liver",
  "Lungs",
  "Nose ",
  "Eye",
];
List<String> Doclottie =[
  'assets/Doc_lottie/Animation - 1708335879503.json',
  "assets/Doc_lottie/Animation - 1708336077598.json",
  'assets/Doc_lottie/Animation - 1708335879503.json',
  "assets/Doc_lottie/Animation - 1708336077598.json",
  'assets/Doc_lottie/Animation - 1708335879503.json',
  "assets/Doc_lottie/Animation - 1708336077598.json",

];
List<String> testlottie=[
  'assets/Doc_lottie/Animation - 1708337998537.json',
  "assets/Doc_lottie/Animation - 1708336077598.json",
  "assets/Doc_lottie/Animation - 1708337935268.json",
  "assets/Doc_lottie/Animation - 1708336077598.json",
  'assets/Doc_lottie/Animation - 1708337998537.json',
  "assets/Doc_lottie/Animation - 1708336077598.json",
];